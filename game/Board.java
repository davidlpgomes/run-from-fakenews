...
public class Board {
  ...
  public Position getRandomEmptyAdjacentPosition(Position pos) {
    ...
    // North
    if (x - 1 >= 0 && this.getPosition(x - 1, y) == null) possiblePositions.add(
      new Position(x - 1, y)
    );

    // South
    if (x + 1 < s && this.getPosition(x + 1, y) == null) possiblePositions.add(
      new Position(x + 1, y)
    );

    // West
    if (y - 1 >= 0 && this.getPosition(x, y - 1) == null) possiblePositions.add(
      new Position(x, y - 1)
    );

    // East
    if (y + 1 < s && this.getPosition(x, y + 1) == null) possiblePositions.add(
      new Position(x, y + 1)
    );

    // Northwest
    if (
      x - 1 >= 0 && y - 1 >= 0 && this.getPosition(x - 1, y - 1) == null
    ) possiblePositions.add(new Position(x - 1, y - 1));

    // Northeast
    if (
      x - 1 >= 0 && y + 1 < s && this.getPosition(x - 1, y + 1) == null
    ) possiblePositions.add(new Position(x - 1, y + 1));

    // Southwest
    if (
      x + 1 < s && y - 1 >= 0 && this.getPosition(x + 1, y - 1) == null
    ) possiblePositions.add(new Position(x + 1, y - 1));

    // Southeast
    if (
      x + 1 < s && y + 1 < s && this.getPosition(x + 1, y + 1) == null
    ) possiblePositions.add(new Position(x + 1, y + 1));
  ...
  public ArrayList<Position> getFakeNewsAdjFakeNews(Position pos) {
    ...
    // North
    if (
      x - 1 >= 0 && this.getPosition(x - 1, y) instanceof FakeNews
    ) possiblePositions.add(new Position(x - 1, y));

    // South
    if (
      x + 1 < s && this.getPosition(x + 1, y) instanceof FakeNews
    ) possiblePositions.add(new Position(x + 1, y));

    // West
    if (
      y - 1 >= 0 && this.getPosition(x, y - 1) instanceof FakeNews
    ) possiblePositions.add(new Position(x, y - 1));

    // East
    if (
      y + 1 < s && this.getPosition(x, y + 1) instanceof FakeNews
    ) possiblePositions.add(new Position(x, y + 1));
  ...
  public void printBoard(History history) {
    ...
    for (int i = 0; i < this.getSize(); i++) {
      ...
      if (history.get(i) != null) System.out.printf("  ● %s", history.get(i));
    ...
    System.out.print("    ");
    for (int i = 65; i < this.getSize() + 65; i++) System.out.printf(
      "%c    ",
      i
    );
